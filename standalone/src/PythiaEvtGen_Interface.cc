...
void PythiaEvtGen_Interface::add_inclusive(std::string decayfile, int seed)
{
  ...
  if(debug) std::cout<<"Creating EvtGen instance"<<std::endl;
...
bool PythiaEvtGen_Interface::check_Signal_Appereance()
{
  ...
  for (int iPro = 0; iPro < event.size(); ++iPro)
    {
      ...
      if(part->isFinal()== false) continue; // skipping the particles that already decayed
      //if (incIDs.find(part->id()) == incIDs.end()) continue; // particle not in the inclusive ID, continue
      if (part->status() == 93 || part->status() == 94) continue; // particles handed by external program

      if( std::find(motherIDs.begin(), motherIDs.end(),abs(part->id()) ) != motherIDs.end() )// the ID is part of the signal to be decayed
	{
  ...
  for(unsigned i=0; i < NOfSignal_list.size(); i++)
    {
      if(NOfSignal_list[i]>0) sum+=1;
    }
  if(debug) std::cout<<"Found "<<sum<<" signal candidates"<<std::endl;
...
void PythiaEvtGen_Interface::decay_signals()
{
  ...
  if(debug) std::cout<<"In decay_signals"<<std::endl;
  unsigned NOfSignal=motherIDs.size();
  for (int i_sig=0; i_sig< NOfSignal ; i_sig++)// loop over signal B's
    {
      ...
      std::cout << "i  " << i_sig << "  sig_names[i_sig]  " << sig_names[i_sig] << std::endl;
...
void PythiaEvtGen_Interface::decay()
{
  ...
  if(IsSignal == false) // repeat hadronization
      ...
      do
	{
      ...
      if(debug) std::cout<<"We have an event after regeneration:"<<std::endl;

    }
  else
    {
      if(debug) std::cout<<"We have an event:"<<std::endl;
    }
  // now we have in the event all the B's that are needed to be decayed
  // first decay signal:
  decay_signals();
  if(debug) std::cout<<"Decayed only signal:"<<std::endl;
  if(debug) pythia->event.list();

  if(debug) std::cout<<"Decaying rest:"<<std::endl;
  ...
  for (int iPro = 0; iPro < event.size(); ++iPro) {
    ...
    if(debug) std::cout<<"Trying to decay ''background'' : "<<part->id()<<" index: "<<iPro<<std::endl;
    // here we have inclusive
    EvtParticle *Evtpart = EvtParticleFactory::particleFactory
      (EvtPDL::evtIdFromStdHep(part->id()),
       EvtVector4R(part->e(), part->px(), part->py(), part->pz()));
    Evtpart->setDiagonalSpinDensity();
    evtgen->generateDecay(Evtpart);
    part->tau(Evtpart->getLifetime());
    UpdatePythiaEvent(part, Evtpart);
  ...
  if(verbose) std::cout<<"EvtGen has finished with the following record:"<<std::endl;
