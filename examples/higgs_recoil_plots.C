...
template <class T> double inv_mass(const T& p1, const T& p2) {
...
template <class T> TLorentzVector v4(const T& p) {
...
void higgs_recoil_plots(const char* FILEN) {
  ...
  std::cout << std::endl << "  " << nEvents << " events read from file: " << FILEN << std::endl;
}
